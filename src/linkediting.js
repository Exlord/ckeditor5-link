...
export default class LinkEditing extends Plugin {
	 ...
	 *
	 * Also registers an
	 * {@link module:engine/conversion/downcasthelpers~DowncastHelpers#attributeToElement attribute-to-element}
	...
	_enableManualDecorators(manualDecoratorDefinitions) {
		...
		manualDecoratorDefinitions.forEach(decorator => {
			...
			editor.conversion.for('downcast').attributeToElement({
				...
				view : (manualDecoratorName, writer) => {
					if (manualDecoratorName) {
						const attributes = manualDecorators.get(
							decorator.id).attributes;
						const element    = writer.createAttributeElement(
							'a', attributes, { priority: 5 });
						writer.setCustomProperty('link', true,
							element);
			...
			editor.conversion.for('upcast').elementToAttribute({
				view : {
					name      : 'a',
					attributes: manualDecorators.get(
						decorator.id).attributes
				},
				model: {
					key: decorator.id
